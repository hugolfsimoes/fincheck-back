...
import { ValidadeCategoryOwnershipService } from 'src/modules/categories/services/validate-category-ownership.service';
...
export class TransactionsService {
  constructor(
    ...
    private readonly validadeCategoryOwnershipService: ValidadeCategoryOwnershipService, private readonly validadeTransactionOwnershipService: ValidadeTransactionOwnershipService) {}
  ...
  async update(userId: string, transactionId: string, updateTransactionDto: UpdateTransactionDto) {
    const { categoryId, bankAccountId, date, name, type, value } = updateTransactionDto;
    ...
    return this.transactionsRepo.update({
      ...
      data: { categoryId, bankAccountId, date, name, type, value }
    });
  }

  async remove(userId, transactionId: string) {
  ...
  private async validateEntitiesOwnership({ userId, bankAccountId, categoryId, transactionId }:
    ...
    await Promise.all([
      transactionId && this.validadeTransactionOwnershipService.validate(userId, transactionId),
      bankAccountId && this.validadeBankAccountOwnershipService.validate(userId, bankAccountId),
      categoryId && this.validadeCategoryOwnershipService.validate(userId, categoryId)
    ]);
